...
public interface BonDeLivraisonDetailRepository extends JpaRepository<BonDeLivraisonDetailEntity, Long> {
    ...
    @Query(value = "SELECT COALESCE(SUM(a.quantite * a.prix_unit_ht), 0) FROM gestion_facturation.tbl_bon_de_livraison_detail a WHERE a.id_bon_de_livraison=?1", nativeQuery = true)
